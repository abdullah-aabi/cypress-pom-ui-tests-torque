...
describe('All the test cases of Estimate CRUD operations', () => {
  ...
  it('Add Misc items to the Job and assert calculations with percentage Discount.', () => {
    const jobCount = 0
    ...
    cy.wait('@getJobsOfEstimate')
      ...
      .then((getJobsResponse) => {
        ...
        let retailPrice = getJobsResponse.body.data.ro.jobs[jobCount].miscellaneousCharges;
        ...
        cy.get(EstimateLocators.jobs)
          ...
          .within(() => {
            ...
            cy.get(EstimateLocators.jobTotals.hoursTotal).invoke("text").then(hoursTotal => {
              ...
              cy.log(hoursTotal)
              addItems("misc", "percentDiscCase", lineItemsCount, parseFloat(hoursTotal), retailPrice, discount, taxTotal, jobTotal);
  ...
  it('Add Misc items to the Job and assert calculations with Fixed Discount.', () => {
    const jobCount = 1
        ...
        cy.get(EstimateLocators.jobs)
          ...
          .within(() => {
            ...
            cy.get(EstimateLocators.jobTotals.hoursTotal).invoke("text").then(hoursTotal => {
              cy.get(EstimateLocators.jobToggle).click();
              cy.log(hoursTotal)
              addItems("misc", "fixedDiscCase", lineItemsCount, parseFloat(hoursTotal), retailPrice, discount, taxTotal, jobTotal);
              // addItems("part", 7) // Parts And Supplies
              // addItems("misc", 7) // Misc
            })
          });
